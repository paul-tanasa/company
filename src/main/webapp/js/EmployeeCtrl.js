...
app.controller('myCtrl',function($scope, $http, $filter,$sce, $timeout, NgTableParams) {
		...
		$scope.removeRow = function ($index) {
			...
			$http({url: "http://localhost:8081/company/Employee/delete", 
				   ...
				   $("#serverResponse").css({"color":"rgb(46, 204, 113)"});
		...
		$scope.submitEmployee = function(empl){
			if($scope.flag=="add"){
				$http({url: "http://localhost:8081/company/Employee/add", 
					   method: "POST",
					   data: empl
				}).then(function(response){
					   console.log("Success at submit!");
					   $("#serverResponse").css({"color":"rgb(46, 204, 113)"});
					   $("#serverResponse").text("Success at submit!");
										 
				},function(response){
					   console.log("Failure at submit!");
					   $("#serverResponse").css({"color":"rgb(46, 204, 113)"});
					   $("#serverResponse").text("Failure at submit!");
				});
				...
				$http({url: "http://localhost:8081/company/Employee/put", 
					   method: "PUT",
					   data: empl
				}).then(function(response){
					   console.log("Success at edit!");
					   $("#serverResponse").css({"color":"rgb(46, 204, 113)"});
					   $("#serverResponse").text("Success at edit!");
					   
										 
				},function(response){
					   console.log("Failure at edit!");
					   $("#serverResponse").css({"color":"red"});
					   $("#serverResponse").text("Failure at edit!");
				});
